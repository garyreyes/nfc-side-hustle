...
export async function getBusinessScanTotals(
  ...
  return db
    ...
    .leftJoin(
      scanEvents,
      and(eq(scanEvents.plateId, plates.id), gte(scanEvents.scannedAt, startAt))
    )
...
export async function getScanTimeSeries(
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(plates.businessId, businessId),
...
// Derived from the schema's enum, not hand-duplicated, so a future
// capability value shows up here automatically instead of silently
// vanishing from the breakdown (see ARCHITECTURE.md: entity definitions
// live only in lib/db/schema.ts).
const CAPABILITIES = capabilityEnum.enumValues;

export async function getScanBreakdownByCapability(
  businessId: string,
  startAt: Date = DASHBOARD_DATA_START_AT
): Promise<CapabilityBreakdown[] | null> {
  if (!(await businessExists(businessId))) return null;
  if (!(await sessionCanAccessBusiness(businessId))) return null;
  ...
  const rows = await db
    ...
    .where(eq(plates.businessId, businessId))
...
export type BranchScanBreakdown = {
  /** null represents plates not attached to any branch. */
...
export async function getBranchScanBreakdown(
  ...
  // Rooted at `branches`, not `plates`, so a branch with no plate yet
  // still shows up with a 0 total — same completeness reasoning as
  // CAPABILITIES above (a real thing that exists shouldn't silently
  // vanish just because it has no traffic yet).
  const branchRows = await db
    ...
    .from(branches)
    .leftJoin(plates, eq(plates.branchId, branches.id))
    .leftJoin(
      scanEvents,
      and(eq(scanEvents.plateId, plates.id), gte(scanEvents.scannedAt, startAt))
    )
  ...
  // A separate query for plates with no branch at all — these can't be
  ...
  // the numbers to add up to what the "By capability" table already shows.
  const [noBranchRow] = await db
    ...
    .where(and(eq(plates.businessId, businessId), isNull(plates.branchId)));
